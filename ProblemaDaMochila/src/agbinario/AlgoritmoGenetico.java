...
public class AlgoritmoGenetico {
    ...
    public AlgoritmoGenetico(Integer tamanho, Double pCrossover, Double pMutacao, Integer geracoes, Problema problema, Integer precisao, Double minimo, Double maximo, Integer nVariaveis, Integer maxPesos) {
    ...
    public void executar() {
        ...
        populacao = new Populacao(precisao, minimo, maximo, nVariaveis, tamanho, problema,maxPesos);
        novaPopulacao = new Populacao(precisao, minimo, maximo, nVariaveis, tamanho, problema,maxPesos);
        ...
        for (int g = 1; g <= geracoes; g++) {
            ...
            for (int i = 0; i < this.tamanho; i++) {
                ...
                if (rnd.nextDouble() <= this.pCrossover) {
                    ...
                    Individuo desc1 = new Individuo(precisao, minimo, maximo, nVariaveis,maxPesos);
                    Individuo desc2 = new Individuo(precisao, minimo, maximo, nVariaveis, maxPesos);
                    ...
                    // Ponto de corte
                    int corte = rnd.nextInt(p1.getCromossomos().size()-1);
        ...
        System.out.println(populacao.getIndividuos().get(0).getCromossomos()+ "  Peso carregado: "+ populacao.getIndividuos().get(0).getFuncaoObjetivo());
        System.out.println(populacao.getIndividuos().get(0).getPesos());
    }
    ...
    private void crossoverUmPonto(Individuo ind1, Individuo ind2, Individuo descendente, int corte) {
        ...
        descendente.getPesos().addAll(ind1.getPesos().subList(0, corte));

        // Ind2_2
        descendente.getCromossomos()
                .addAll(ind2.getCromossomos().subList(corte, ind2.getCromossomos().size()));
        descendente.getPesos().addAll(ind2.getPesos().subList(corte, ind2.getPesos().size()));
    }
